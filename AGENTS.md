...
- `workflow/scripts/build_root_pages.ts` turns the root Markdown into `dist/index.html` and `dist/domain-glossary.html`.
- `workflow/scripts/build_tag_guides_html.ts` turns each tag-guide Markdown file into a matching HTML file in `dist/tag-guides/`.
- `workflow/scripts/package_dist_assets.ts` packages site CSS / JS into `dist/site/` for local preview and GitHub Pages artifact use.
